...
# evaluate/eval_real_time_API_HLP_qwen.py  (발췌: 새로 추가)
...
@dataclass
class HLPClientConfig:
    url: str = "http://127.0.0.1:8787"
    timeout: float = 15.0

class HighLevelPlannerRemote:
    def __init__(self, cfg: HLPClientConfig):
        self.url = cfg.url.rstrip("/")
        self.timeout = cfg.timeout

    def infer(self, task: str, prev_desc: str, prev_status: str, side_pil: Image.Image, wrist_pil: Image.Image):
        payload = {
            "task": task, "prev_desc": prev_desc, "prev_status": prev_status,
            "side_b64": _pil_to_b64(side_pil),
            "wrist_b64": _pil_to_b64(wrist_pil),
        }
        r = requests.post(self.url + "/infer", json=payload, timeout=self.timeout)
        r.raise_for_status()
        return r.json()  # {desc_1, desc_2, status, subtask, raw_text, latency_ms}
