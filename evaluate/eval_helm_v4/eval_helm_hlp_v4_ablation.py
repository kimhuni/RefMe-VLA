...
CUDA_VISIBLE_DEVICES=5 python evaluate/eval_helm_v4/eval_helm_hlp_v4.py \
  --jsonl /data/ghkim/helm_data/helm_ablation_task_5/merged/all_val.jsonl \
  --base_model /ckpt/Qwen2.5-VL-7B-Instruct \
  --adapter /backups/ghkim/HeLM_v4/HLP_HeLM_v4_qwen_7b_ablation_task_5/checkpoint-1000 \
  --batch_size 2 \
  --max_new_tokens 128 \
  --max_samples 200 \
  --out_jsonl /data/ghkim/helm_data/helm_ablation_task_5/test/eval_preds_step_1k.jsonl
...
def _load_images(row: Dict[str, Any], num_images: int) -> List[Image.Image]:
    """Load 1 or 2 images from a row.

    Supports both:
      - images: {"table": "/path", "wrist": "/path"}
      - images: [ {"table": "/path"}, ... ] or ["/path", ...]
    """

    def _get_path(images: Any, key: str) -> Optional[str]:
        ...
        if isinstance(images, list):
            ...
            for it in images:
                if isinstance(it, dict) and isinstance(it.get(key), str):
                    return it[key]
            # list of strings (best-effort: first is table, second is wrist)
            strs = [it for it in images if isinstance(it, str)]
            if not strs:
                return None
            if key == "table":
                return strs[0]
            if key == "wrist" and len(strs) >= 2:
                return strs[1]
            return None
        return None
    ...
    table = _get_path(images, "table")
    if not table:
        raise ValueError("row['images'] missing a usable 'table' path")
    imgs.append(Image.open(table).convert("RGB"))

    if num_images == 2:
        wrist = _get_path(images, "wrist")
        if not wrist:
            raise ValueError("num_images=2 but row['images'] missing a usable 'wrist' path")
...
def load_model_and_processor(
    ...
    device_map = None
    # For 4bit/8bit models, load directly onto the target device.
...
def run_eval(
    ...
    detect_stats = defaultdict(lambda: {"n": 0, "ok": 0})
    ...
    for step, batch in enumerate(dl):
        ...
        for i in range(len(batch["uids"])):
            ...
            if mode_u == "DETECT":
                ...
                detect_stats["DETECT"]["n"] += 1
                detect_stats["DETECT"]["ok"] += int(ok)
            elif mode_u == "STEP":
                # Unified ablation: must match BOTH event flag and memory fields.
                ok_e, info_e = eval_detect(gt_yaml, pred_yaml)
                ok_u, info_u = eval_update(gt_yaml, pred_yaml)
                ok = bool(ok_e and ok_u)
                info = {
                    "match_event": ok_e,
                    **info_e,
                    **info_u,
                }
                detect_stats["STEP"]["n"] += 1
                detect_stats["STEP"]["ok"] += int(ok_e)
                # per-key stats
                for k in ["Action_Command", "Working_Memory", "Episodic_Context"]:
                    update_key_stats[k]["n"] += 1
                    update_key_stats[k]["ok"] += int(info_u.get(f"match_{k}", False))
            else:
    ...
    if detect_stats:
        for m, d in sorted(detect_stats.items(), key=lambda x: x[0]):
            logger.info(f"[EVENT {m}] n={d['n']} acc={acc(d):.2f}%")
    ...
    if detect_stats:
        for m, d in sorted(detect_stats.items(), key=lambda x: x[0]):
            logger.info(f"[EVENT {m}] n={d['n']} acc={(d['ok']/max(d['n'],1))*100.0:.2f}%")
