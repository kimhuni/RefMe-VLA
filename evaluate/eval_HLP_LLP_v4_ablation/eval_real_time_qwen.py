...
def parse_step_yaml(text: str) -> Dict[str, Any]:
    ...
    out = {
        ...
        "Action_Command": str(d.get("Action_Command", "") or "").strip(),
        "Working_Memory": str(d.get("Working_Memory", "") or "").strip(),
        "Episodic_Context": str(d.get("Episodic_Context", "") or "").strip(),
    }
    return out
...
class HLPQwenV2:
    ...
    def step(self, batch: Dict[str, torch.Tensor]) -> Dict[str, Any]:
        # unified STEP는 update 길이(128) 정도를 쓰는 게 안정적
        out_text = self._generate_text(batch, self.max_new_tokens_update)
        print("\n[STEP] raw output\n", out_text)
        print("---------------------------------------------------")
        return parse_step_yaml(out_text)