...
class AlternatingBatchSampler(BatchSampler):
    def __init__(self, dataset, detect_batch_size=8, update_batch_size=1, seed=42):
        ...
        # 배치 개수 계산
        self.n_detect_batches = (len(self.detect_indices) + detect_batch_size - 1) // detect_batch_size if detect_batch_size > 0 else 0
        self.n_update_batches = (len(self.update_indices) + update_batch_size - 1) // update_batch_size if update_batch_size > 0 else 0
