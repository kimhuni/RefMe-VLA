...
class TaskSpecV4:
    ...
    event_grid: List[List[Dict[str, str]]]      # [inter][state] (len = intra[inter] + 1)
    llp_commands: str = ""
    event_list: str = ""
    ...
    def validate(self) -> None:
        ...
        if self.event_grid is not None:
            if len(self.event_grid) != (self.inter + 1):
                raise ValueError(f"[{self.task_id}] event_grid length mismatch.")
            for i in range(self.inter + 1):
                exp_steps = self.intra[i]
                if len(self.event_grid[i]) != exp_steps:
                    raise ValueError(
                        f"[{self.task_id}] event_grid[{i}] steps mismatch: "
                        f"{len(self.event_grid[i])} vs {exp_steps}"
                    )
                for e in self.event_grid[i]:
                    if not isinstance(e, str):
                        raise ValueError(f"[{self.task_id}] event_grid entries must be strings.")
