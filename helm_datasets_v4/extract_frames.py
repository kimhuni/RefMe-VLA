...
python -m helm_datasets_v3.extract_frames \
  --lerobot_root "/data/libero-mem_lerobot_5hz/ep20/pickplace_creamcheese_right_ep20" \
  --out_root     "/data/libero-mem_lerobot_5hz/helm_data/pickplace_creamcheese_right" \
  --fps_frames 5
"""
...
def write_episode_meta(
    ...
    p = episode_json_path(out_root, fps_frames, chunk, ep_id)
    ensure_dir(p.parent)
...
def extract_frames_at_fps(mp4_path: Path, out_dir: Path, overwrite: bool, fps_out: float) -> int:
    """
    Extract frames from mp4 at fps_out frames per second and save.
    - Saved filename: frame_{index:06d}.jpg (index=0,1,2,...)
    """
    ensure_dir(out_dir)
    ...
    step_sec = 1.0 / fps_out
    num_steps = int(duration_sec * fps_out)

    saved = 0
    for i in range(num_steps + 1):
        cap.set(cv2.CAP_PROP_POS_MSEC, i * step_sec * 1000.0)
        ok, frame = cap.read()
        if not ok:
            continue
...
def main():
    ...
    for chunk in chunks:
        ...
        for ep_id in episodes:
            ...
            out_table = frames_dir(out_root, fps_out, chunk, ep_id, "table")
            out_wrist = frames_dir(out_root, fps_out, chunk, ep_id, "wrist")

            n1 = extract_frames_at_fps(table_mp4, out_table, overwrite=overwrite, fps_out=fps_out)
            n2 = extract_frames_at_fps(wrist_mp4, out_wrist, overwrite=overwrite, fps_out=fps_out)
    ...
    print(f"Done. processed episodes: {total_eps}. Saved at: {out_root}")
