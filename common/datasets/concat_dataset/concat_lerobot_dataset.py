# tools/concat_lerobot_from_recipes.py
"""
...
Recipe format (JSONL, one per line):
{
  ...
  "boundary_policy": {"noop_frames": 0, "drop_last_frame_each_part": true}
}
...
"""
python tools/concat_lerobot_from_recipes.py \
  --src_root /data/ghkim/wipe_the_window_ep150 \
  --recipes recipes_300.jsonl \
  --out_root /data/ghkim/wipe_the_window_ep150_concat_abc \
  --fps 5.0 \
...
def out_parquet_path(out_root: str, new_episode_index: int, chunk_size: int = 1000) -> str:
...
def out_video_path(out_root: str, new_episode_index: int, video_key: str, chunk_size: int = 1000) -> str:
    out_chunk = new_episode_index // chunk_size
...
def main(
    ...
    for r in recipes:
        ...
        ok += 1
        print(f"[OK] new_ep={new_ep:06d} frames={len(cat_df)} parts={len(parts)} noop={noop_frames} drop_last={drop_last_frame_each_part}")
