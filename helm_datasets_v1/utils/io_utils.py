...
def list_chunks_from_frames(out_root: Path) -> List[str]:
    """
...
def list_episodes_from_frames(out_root: Path, chunk: str) -> List[str]:
    """
    frames_1hz/<chunk>/episode_* 폴더 목록
    """
    base = out_root / "frames_1hz" / chunk
    if not base.exists():
        return []
...
def frames_dir(out_root: Path, chunk: str, episode: str, camera: str) -> Path:
    return out_root / "frames_1hz" / chunk / episode / camera
...
def update_index_event(out_root: Path, chunk: str, episode: str, boundaries: List[int]) -> None:
    ...
    """
    ep_json = episode_json_path(out_root, chunk, episode)
