...
def _prompt_v4_2_partially_done(task: str, prev: str, prev_status: str) -> str:
    return (
        "SYSTEM:\n"
        "- Return ONE-LINE JSON ONLY: {\"desc\":\"...\",\"status_reasoning\":\"...\",\"status\":\"DONE|NOT_DONE|PARTIALLY_DONE\"}\n"
        "- DO NOT copy PREV_STATUS_REASONING verbatim. If you need to refer to it, summarize it in ≤8 words.\n"
        ...
        "(robot completely out of frame, strong blur, heavy occlusion).\n"
        "- When gripper pose, alignment, or contact changes, you MUST update status_reasoning, "
        "but you MAY stay on the same press/subtask if the previous one was not clearly completed.\n"
        "- IMPORTANT: Phrases like 'attempted but not completed' NEVER count as a finished press. "
        "You must NOT advance to 'second press' or 'third press' unless the previous press is clearly completed "
        "either in PREV_STATUS_REASONING or in the CURRENT image.\n"
        "- Sticky DONE: If PREV_STATUS is DONE, keep DONE unless a clear reversal is visible.\n"
        "- Completion evidence does NOT require large deformation or bright illumination. "
        "Small downward movement, stable aligned contact, or subtle displacement CAN count as a completed press "
        "when consistent with the task.\n"
        "- However, do NOT assume a new press index (second/third) purely from small pose changes. "
        "If you are unsure whether the previous press finished, keep describing the same press attempt.\n"
        "- Avoid overly conservative NOT_DONE; prefer PARTIALLY_DONE when the image shows clear purposeful contact "
        "toward the goal, but keep the press index (first / second / third) consistent.\n"
        "\n"
        "USER:\n"
        "You are an image-analysis expert for robotic manipulation.\n"
        "Inputs:\n"
        ...
        "1) desc = concise visible description (mention TABLE/WRIST when relevant; no speculation).\n"
        "2) status_reasoning = updated progress based on CURRENT image: "
        "use phrases like 'first press attempted', 'first press completed; attempting second', "
        "'second press completed; attempting third'. "
        "Do NOT say 'second press attempted' if the previous reasoning explicitly said the first press "
        "was 'attempted but not completed'. In that case, continue to describe the first press.\n"
        "\n"
        "Status rules (DONE/NOT_DONE/PARTIALLY_DONE):\n"
        "- DONE: all subtasks/presses visibly completed in order.\n"
        "- PARTIALLY_DONE: ≥1 subtask/press completed but not all.\n"
...
def _prompt_v5_structured(task: str, prev: str, prev_status: str) -> str:
    return (
        "SYSTEM:\n"
        "- Return ONE-LINE JSON ONLY: {\"desc\":\"...\",\"status_reasoning\":\"...\",\"status\":\"DONE|NOT_DONE|PARTIALLY_DONE\"}.\n"
        "- status_reasoning MUST be a SINGLE sentence with this exact template:\n"
        "  \"PROGRESS[...] ; EVIDENCE[...] ; PRIOR[...]\".\n"
        "- PROGRESS[...] = short progress tag (e.g., \"0/2\", \"1/2\", \"2/2\" or \"first press done\", \"no press yet\").\n"
        "- EVIDENCE[...] = key CURRENT visual cues from TABLE/WRIST (contact, alignment, small displacement, release, motion halt).\n"
        "- PRIOR[...] = ≤8-word summary of PREV_STATUS_REASONING or \"first frame\".\n"
        "- DO NOT copy PREV_STATUS_REASONING verbatim; always compress it in PRIOR[...].\n"
        "- Only reuse previous reasoning/status when the image is genuinely unusable "
        "(robot completely out of frame, strong blur, heavy occlusion).\n"
        "- When gripper pose, alignment, or contact clearly changes compared to PRIOR, "
        "you MUST update PROGRESS and EVIDENCE to reflect the new attempt/progress "
        "(e.g., \"PROGRESS[1/2]\" → \"PROGRESS[2/2]\").\n"
        "- Status changes should be conservative and mostly monotonic:\n"
        "  * Keep the same status unless there is strong new evidence.\n"
        "  * NEVER downgrade status (e.g., PARTIALLY_DONE→NOT_DONE, DONE→PARTIALLY_DONE) "
        "    unless a clear reversal is visible (e.g., object removed, button obviously released).\n"
        "- Sticky DONE: If PREV_STATUS is DONE, keep DONE unless a clear reversal is visible.\n"
        "- Completion evidence does NOT require large deformation or bright illumination; "
        "small downward movement, stable aligned contact, or subtle displacement CAN count as a completed press "
        "when consistent with the task and not contradicted by the image.\n"
        "- Avoid overly sensitive NOT_DONE flips; if progress is ambiguous but clearly purposeful towards the goal, "
        "prefer PARTIALLY_DONE over NOT_DONE.\n"
        "\n"
        "USER:\n"
        "You are an image-analysis expert for robotic manipulation.\n"
        "Inputs:\n"
        ...
        f"- PREV_STATUS: {prev_status}\n"
        "- IMAGES: TABLE (global view), WRIST (close-up contact view).\n"
        "\n"
        "Write EXACTLY TWO sentences:\n"
        "1) desc = concise visible description of the CURRENT image (8–16 words; mention TABLE/WRIST when relevant; no speculation).\n"
        "2) status_reasoning = one sentence following EXACTLY the template:\n"
        "   \"PROGRESS[...] ; EVIDENCE[...] ; PRIOR[...]\".\n"
        "   - PROGRESS: how many subtasks/presses appear completed now (parse the count from TASK if possible).\n"
        "   - EVIDENCE: the most important CURRENT visual cues that justify keeping or changing the status.\n"
        "   - PRIOR: a ≤8-word summary of the previous frame's reasoning or simply \"first frame\".\n"
        "\n"
        "Status rules (DONE/NOT_DONE/PARTIALLY_DONE):\n"
        "- DONE: all required subtasks/presses visibly completed in correct order.\n"
        "- PARTIALLY_DONE: ≥1 subtask/press completed but not all.\n"
        "- NOT_DONE: no verified completion.\n"
        "\n"
        "Constraints:\n"
        "- desc and status_reasoning must both be non-empty and follow the structure above.\n"
        "- Use only visible cues (contact, slight displacement, alignment, release, motion halt, stable endpoint) from TABLE/WRIST.\n"
        "\n"
        "OUTPUT JSON ONLY on one line:\n"
        "{\"desc\":\"...\",\"status_reasoning\":\"...\",\"status\":\"DONE|NOT_DONE|PARTIALLY_DONE\"}"
    )
...
PROMPTS = {
    ...
    "eval_v4_2_partially_done": _prompt_v4_2_partially_done,
    "eval_v5_structured": _prompt_v5_structured,
}
